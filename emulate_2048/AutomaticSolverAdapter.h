...
namespace Game_2048 {
	 ...
	 *
	 * This type has no particular reason to be copyable
	 * But I want it to be movable, so I'll use a unique_ptr<mutex> instead of mutex
	 */
	class AutomaticSolverAdaptor
	{
	public:
	...
	private:
			...
			pImpl_{ std::move(pImpl) }
		{
			game_is_over_ =
				std::async(
					std::launch::async, // I have to insist on async, because I'll check
										// how it's running
					[this] { start_solving();}
			);
		}
